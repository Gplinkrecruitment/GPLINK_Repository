...
6. **ALWAYS COMMIT AND PUSH — DIRECTLY TO `main`.** After every code change, commit and push immediately. **You are explicitly authorized to push straight to `main`** (`git push origin main`, or `git push origin HEAD:main` from a worktree branch) — this is the owner's intended workflow. Do NOT hand the push back to the owner as a manual `!`-command, and do NOT insist on the Vercel CLI. **Pushing to `main` auto-deploys to production** via the native Vercel↔GitHub integration (~1–2 min; verified 2026-07-26 — see `docs/deployment-pathway.md`). Guardrails (do these, then push — do not ask): work from a **fresh worktree** (the local checkout lags `origin/main` by 100s of commits), **run the tests first** (download a temp Node to `$CLAUDE_JOB_DIR/tmp` if none is on PATH; symlink the main checkout's `node_modules`), stage **specific files** (never `git add -A`), and confirm a clean fast-forward (`git fetch origin main && git merge-base --is-ancestor origin/main HEAD`) before pushing. ⚠️ This is PROD (live Supabase + live public site) — so verify before you push, but then push.
...
- `js/onboarding.js` — 5-step onboarding wizard
...
The Visa application step was briefly deferred but re-enabled in the user flow on 2026-05-12 (see `docs/deferred-visa-application.md` for history).
