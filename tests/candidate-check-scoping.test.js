...
describe('taskVisibleToRso', () => {
  ...
  it('(b) shows a task on an UNASSIGNED case routed to me via assignee', () => {
    const task = { id: 't2', assignee: ME };
    ...
    // ...but the task-level assignee routing reveals it to me.
    expect(taskVisibleToRso(meScope, task, caseRow)).toBe(true);
  });

  it('(c) hides a task on an UNASSIGNED case routed to another RSO', () => {
    const task = { id: 't3', assignee: OTHER };
    const caseRow = { assigned_rso: null, assigned_va: null };
    expect(taskVisibleToRso(meScope, task, caseRow)).toBe(false);
  });

  it('(c2) hides a task on an UNASSIGNED case with no assignee at all', () => {
    const task = { id: 't3b', assignee: null };
    const caseRow = { assigned_rso: null, assigned_va: null };
    expect(taskVisibleToRso(meScope, task, caseRow)).toBe(false);
  });

  it('(d) super-admin sees every task (assigned, routed elsewhere, or orphan)', () => {
    expect(taskVisibleToRso(superScope, { assignee: OTHER }, { assigned_rso: OTHER })).toBe(true);
    expect(taskVisibleToRso(superScope, { assignee: null }, { assigned_rso: null, assigned_va: null })).toBe(true);
  ...
  it('does not leak another RSO’s assigned case when the task has no assignee', () => {
    const task = { id: 't4', assignee: null };
  ...
  it('an admin with no roster identity (rsoUserId null) matches nothing', () => {
    ...
    expect(taskVisibleToRso(noIdScope, { assignee: null }, { assigned_rso: null })).toBe(false);
    expect(taskVisibleToRso(noIdScope, { assignee: ME }, { assigned_rso: null })).toBe(false);
  });

  it('assignee routing does not override case ownership by another RSO', () => {
    // Case is assigned to OTHER but the task happens to carry assignee===ME.
