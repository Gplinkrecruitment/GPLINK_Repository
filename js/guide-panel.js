...
(function () {
  ...
  var CSS = [
    '.gpg-wrap{display:flex;height:calc(100vh - 190px);min-height:460px;overflow:hidden;border:1px solid var(--line,#e5e7eb);border-radius:10px;background:var(--bg2,#f8fafc)}',
    '.gpg-folders{flex:0 0 340px;overflow-y:auto;padding:16px;border-right:1px solid var(--line,#e5e7eb);background:#fff}',
    '.gpg-detail{flex:1;display:flex;flex-direction:column;background:var(--bg2,#f8fafc)}',
    '.gpg-detail-header{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--line,#e5e7eb);background:#fff}',
    '.gpg-detail-header h3{margin:0;font-size:14px;font-weight:700}',
    '.gpg-detail-body{flex:1;overflow:hidden;position:relative}',
    ...
    '.gpg-folder.drag-over-above::before{content:"";position:absolute;top:-4px;left:0;right:0;height:3px;background:var(--blue,#2563eb);border-radius:2px;z-index:10}',
    '.gpg-folder.drag-over-below::after{content:"";position:absolute;bottom:-4px;left:0;right:0;height:3px;background:var(--blue,#2563eb);border-radius:2px;z-index:10}',
    '.gpg-folder-header{display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:8px;cursor:pointer;font-weight:600;font-size:13px;color:var(--text,#0f172a);background:var(--bg2,#f1f5f9);transition:background .15s}',
    '.gpg-folder-header:hover{background:var(--bg3,#e2e8f0)}',
    '.gpg-folder-header.active{background:#e8f0fe;color:var(--blue,#2563eb)}',
    '.gpg-folder-count{background:var(--bg3,#e2e8f0);padding:1px 6px;border-radius:10px;font-size:10px;margin-left:auto;font-weight:700}',
    '.gpg-folder-header.active .gpg-folder-count{background:var(--blue,#2563eb);color:#fff}',
    '.gpg-folder-items{padding:4px 0 4px 24px}',
    '.gpg-item{padding:8px 12px;font-size:12px;color:var(--text,#0f172a);border-radius:6px;cursor:pointer;margin:2px 0;border:1px solid transparent;transition:all .15s;display:flex;align-items:center;gap:4px}',
    '.gpg-item:hover{background:var(--bg2,#f1f5f9);border-color:var(--line,#e5e7eb)}',
    '.gpg-item.active{background:var(--blue,#2563eb);color:#fff;border-color:var(--blue,#2563eb)}',
    '.gpg-empty{display:flex;align-items:center;justify-content:center;height:100%;color:var(--muted,#64748b);font-size:13px;text-align:center;padding:40px}',
    '.gpg-add-btn{display:inline-flex;align-items:center;gap:4px;padding:6px 12px;border-radius:6px;font-size:11px;font-weight:600;color:var(--blue,#2563eb);background:transparent;border:1px dashed var(--blue,#2563eb);cursor:pointer;margin-top:8px;transition:all .15s}',
    '.gpg-add-btn:hover{background:#e8f0fe}',
    '.gpg-mgmt-btn{opacity:0;padding:4px;border:none;background:transparent;cursor:pointer;font-size:12px;color:var(--muted,#64748b);transition:opacity .15s;flex-shrink:0}',
    '.gpg-folder-header:hover .gpg-mgmt-btn,.gpg-item:hover .gpg-mgmt-btn{opacity:1}',
    '.gpg-mgmt-btn:hover{color:var(--red,#e53e3e)}',
    ...
    '.gpg-folders{flex:0 0 auto;max-height:35vh;overflow-y:auto;padding:12px;border-right:none;border-bottom:1px solid var(--line,#e5e7eb)}',
  ...
  Instance.prototype.render = function () {
      ...
      return list.map(function (f) {
        ...
        if (expanded) {
          itemsHtml = '<div class="gpg-folder-items">' +
            ...
            (items.length === 0 && children.length === 0 ? '<div style="padding:8px 12px;font-size:11px;color:var(--muted,#64748b)">No guides in this folder yet</div>' : '') +
    ...
    if (selectedItem) {
      ...
      detailHtml = '<div class="gpg-detail-header"><h3>' + esc(selectedItem.title) + '</h3><button data-gpg-close style="border:none;background:transparent;cursor:pointer;font-size:18px;color:var(--muted,#64748b)">&times;</button></div>' +
