(function () {
  ...
  const ALLOWED_REVIEW_PAGES = ["/pages/myinthealth.html", "/pages/account.html", "/pages/onboarding.html"];
  ...
  function injectRestrictionUI() {
    ...
    popup.id = popupId;
    popup.style.cssText = "position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);";
    popup.innerHTML =
      '<div style="background:#fff;border-radius:20px;padding:32px 24px;max-width:340px;margin:0 20px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.15);">' +
        ...
        '<p style="font-size:14px;color:#64748b;line-height:1.5;margin:0 0 20px;">Your account is under review for discrepancy of qualification information and will be unlocked after verification.</p>' +
    ...
    function interceptNav() {
      ...
      document.querySelectorAll("a.bottom-tab, a.mobile-tab, .sidebar a, nav a").forEach(function (a) {
        ...
        if (!ALLOWED_REVIEW_PAGES.includes(target) && target !== pathname) {
          a.style.opacity = "0.4";
          a.addEventListener("click", showReviewPopup);
        }
      });
      ...
      document.querySelectorAll("button.bottom-tab, button.mobile-tab").forEach(function (btn) {
        ...
        // Allow account tab, block others
        if (label !== "account") {
          btn.style.opacity = "0.4";
          btn.addEventListener("click", showReviewPopup);
        }
      });
      ...
      document.querySelectorAll("[data-qual-scan-trigger]").forEach(function (el) {
        ...
        el.addEventListener("click", showReviewPopup);
      });
    }
