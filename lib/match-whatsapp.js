...
//     live on the same role (server.js counts shortlisted gp_applications on
//     that career_role_id). The count GATES the sentence but is never stated in
//     it — owner call 2026-08-29.
...
function buildUrgencyLine(ctx) {
  ...
  if (others > 0) {
    // Owner 2026-08-29: say that others exist, never HOW MANY. A real number
    // invites arithmetic ("only 2? I have time") and tells a doctor something
    // about the pipeline that is not theirs to know. The count still gates the
    // sentence — see the header — it just no longer appears in it.
    // Singular/plural still tracks the real number so the claim stays true when
    // exactly one other doctor is live.
    const who = others === 1 ? 'Another doctor has' : 'Other doctors have';
    ...
    return who + ' also been shortlisted for this position, and ' + holdClause
      + '. The sooner you accept, the sooner we can put you in front of them.';
  }
