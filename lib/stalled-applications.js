...
function isIntroduced(app) {
  const a = app && typeof app === 'object' ? app : {};
  // NOT `revealed`: match rows are written revealed at match time, long
  // before any practice has met the doctor (review 2026-09-07).
  if (norm(a.practice_submission_status) === 'client_approved') return true;
