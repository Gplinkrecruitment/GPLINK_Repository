...
// 'already_received' is only valid when the completed document is already on file (the
// SPPA-00 state machine says so) — a late "we already sent it" reply on an old reminder
// thread must be answered with an acknowledgement, never a chase. See buildPracticeReplyMessages.
var OUTCOMES = ['delegated', 'content_in_body', 'will_send_later', 'question_for_us', 'cannot_provide', 'already_received', 'unclear'];
...
function outcomeGuidance(outcome) {
  switch (outcome) {
    ...
    case 'already_received':
      return 'The completed form is already on file — nothing needs re-sending. A short acknowledgement closes the loop.';
    default:
...
function buildPracticeReplyMessages(params) {
  ...
  var systemText = [
    ...
    // The email thread can lie about the present: our request and any reminders were written
    // BEFORE the document arrived through another channel, so a model shown only the thread
    // faithfully concludes "we do not appear to have received it" while the completed form sits
    // stored on the task (Dr Mercy Obanimoh, 2026-08-27). This fact outranks the thread.
    '- FACTS includes "completed_document_already_on_file". When it is TRUE, the completed signed document HAS',
    '  already reached us and is safely stored — it arrived separately, after the request below was sent, so the',
    '  thread may wrongly read as though we are still waiting. In that case: use outcome "already_received", the',
    '  summary must state plainly that we already hold the completed document (if they said they had sent it,',
    '  they were right), and the follow-up must thank them and confirm receipt. NEVER ask them to resend it and',
    '  NEVER say or imply we have not received it. When it is FALSE, never use outcome "already_received".',
  ...
  // Show BOTH the header name and the name they sign off with. The header name is frequently a
  // role ("Practice Manager"), and a practice delegating by first name ("Naomi will send it") can
  // only be matched against the personal name — which lives in their signature. Listing one and
  // not the other is how "we do not have her email address" got written about somebody sitting in
  // the CC line. See lib/email-signature-name.js.
  var threadPeople = knownContacts.map(function (c) {
    var email = String((c && (c.email || c.email_address)) || '').trim();
    if (!email) return null;
    var header = String((c && (c.name || c.display_name)) || '').trim();
    var signed = String((c && c.signature_name) || '').trim();
    var label = signed || header;
    if (signed && header && header.toLowerCase() !== signed.toLowerCase()) label = signed + ' (' + header + ')';
  ...
  var facts = {
    ...
    completed_document_already_on_file: !!params.formOnFile,
...
function buildFallbackFollowup(params) {
  ...
  // The completed document is already on file (the SPPA-00 state machine says so): the only
  // correct follow-up is an acknowledgement. Chasing here would ask the practice to resend a
  // form we are holding.
  if (params.formOnFile) {
    var ackBody = 'Hi ' + (contactName || 'there') + ',\n\n'
      + 'Thank you for coming back to us about the ' + docTitle + ' for Dr ' + gpName + '.\n\n'
      + 'Good news: we do have the completed document on file, so there is nothing further we need from you on this one. '
      + 'Apologies for any crossed wires with our earlier reminder.\n\n'
      + 'Kind regards,\n' + (rsoName ? rsoName + ' — GP Link Registration Team' : 'GP Link Registration Team');
    return {
      outcome: 'already_received',
      summary: 'The practice replied to our reminder. The completed ' + docTitle + ' is already on file, so nothing further is needed from them.',
      handed_to: '',
      suggested_subject: 'Re: ' + docTitle + ' for Dr ' + gpName,
      suggested_reply: ackBody,
      rso_notes: '',
      source: 'fallback',
    };
  }
