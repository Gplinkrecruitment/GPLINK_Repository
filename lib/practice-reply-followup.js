...
function buildPracticeReplyMessages(params) {
  ...
  var systemText = [
    ...
    '  and ask for that person\'s email if we do not have it. Do not pretend we can email someone whose address we lack.',
    ...
    '- If they asked us a question, answer it if the facts below allow, otherwise mark it for the RSO with',
    '  "[RSO: please confirm ...]".',
    ...
    '  "handed_to": "name and/or email of the person they delegated to, or empty string",',
    '  "suggested_subject": "subject line for the follow-up",',
    '  "suggested_reply": "the follow-up email body as plain text"',
    '}',
  ].join('\n');
...
function parsePracticeReplyResult(raw) {
  ...
  var body = String(parsed.suggested_reply == null ? '' : parsed.suggested_reply).trim();
  if (!body) return null;
