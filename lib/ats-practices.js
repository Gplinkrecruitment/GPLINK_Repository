...
// Doctor-visibility flag (owner 2026-09-15: "hide GP Link Sandbox Practice
// from any user"): practices.metadata.hidden_from_doctors === true hides every
// role of this practice from doctor-facing LISTINGS — the careers board, the
// map pins and count, the public jobs feed, the job page reached by URL —
// while every staff surface, and a doctor who already holds a match or an
// application on the role, keep working exactly as before. Absent/false/any
// non-boolean = visible.
function isPracticeHiddenFromDoctors(practice) {
  var meta = practice && practice.metadata;
  if (typeof meta === 'string') { try { meta = JSON.parse(meta); } catch (e) { meta = null; } }
  return !!(meta && typeof meta === 'object' && meta.hidden_from_doctors === true);
}
function normalisePracticeNameKey(name) {
  return String(name || '').trim().toLowerCase().replace(/\s+/g, ' ');
}
// refs = { ids: Set<string>, names: Set<string> } — build once per request via
// buildHiddenPracticeRefs; a role is hidden by practice_id, or by practice
// name only when it carries no practice_id at all (legacy rows).
function buildHiddenPracticeRefs(practices) {
  var ids = new Set(), names = new Set();
  (Array.isArray(practices) ? practices : []).forEach(function (p) {
    if (!isPracticeHiddenFromDoctors(p)) return;
    if (p.id != null) ids.add(String(p.id));
    var n = normalisePracticeNameKey(p.name);
    if (n) names.add(n);
  });
  return { ids: ids, names: names };
}
function roleBelongsToHiddenPractice(role, refs) {
  if (!role || !refs) return false;
  var pid = role.practice_id != null && role.practice_id !== '' ? String(role.practice_id) : '';
  if (pid) return !!(refs.ids && refs.ids.has(pid));
  var name = normalisePracticeNameKey(role.practice_name);
  return !!(name && refs.names && refs.names.has(name));
}
...
module.exports = {
  ...
  isPracticeDeleted,
  isPracticeHiddenFromDoctors,
  buildHiddenPracticeRefs,
  roleBelongsToHiddenPractice,
  practicePurgeDue,
