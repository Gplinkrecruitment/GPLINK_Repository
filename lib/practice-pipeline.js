...
// Practice reveal TIERS (owner decision 2026-09-04: doctors want the practice
// name and website BEFORE applying).
//   'identity' — the full reveal above: real name, website, street address,
//                exact map, contact name.
//   'named'    — a signed-in doctor who has passed the CV gate (a verified
//                careers CV on file) sees the practice NAME and WEBSITE only.
//                Address, exact map and contact details still wait for
//                'identity'. Never on the public jobs board.
//   'masked'   — everything else (no CV yet, anonymous, admin preview).
function practiceRevealTier({ application, offer, cvVerified } = {}) {
  if (canRevealPracticeIdentityCore({ application, offer })) return 'identity';
  if (cvVerified === true) return 'named';
  return 'masked';
}
