 ...
 * Practice Plan Agreement). Runs just before an RSO submits the form to AHPRA:
...
var COMPLETENESS_SYSTEM_PROMPT = [
  ...
  'You are given a COMPLETED "Supervised practice plan" form (form code SPPA-00, 13 pages) that a',
  'medical practice has filled in and signed, plus an inventory of the supporting documents GP Link',
  'currently holds. Your job: decide whether this form is READY TO SUBMIT to AHPRA, or whether',
  ...
  '- If Q17 = YES: the Q18 supervised-practice goals/activities template must be attached.',
...
async function checkSppaCompleteness(params, opts) {
  ...
  var result = await callAnthropicMessages({
    max_tokens: 1200,
  ...
  }, { apiKey: apiKey, timeoutMs: opts.timeoutMs || 90000 });
