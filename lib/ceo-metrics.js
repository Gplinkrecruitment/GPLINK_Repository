...
function computeConversionFunnel(data, period, nowMs) {
  ...
  var practicesSigned = practices.filter(function(p) {
    return p.agreement_status === 'signed' && inP(p.agreement_signed_at || p.created_at);
  }).length;
...
module.exports = {
  ...
  computeBlockers,
  computeTaskHealth,
  computeRsoWorkload, rsoCaseIds, resolveRsoReassignmentTarget, opsTasksForRso, supportTicketsForRso, callsForRso, gpTasksByStage,
