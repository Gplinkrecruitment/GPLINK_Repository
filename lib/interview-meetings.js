...
// The bookable window, and the SINGLE source of truth for it. Three places used
// to state it independently and two of them disagreed: the practice's date
// picker offered up to 60 days, the server accepted up to 60 days, and the
// scheduler only ever looked at the next 14 — so a practice could pick a date
// three weeks out, be told it was received, and have it silently ignored by
// everyone. The same held at the near end: anything inside the 48-hour notice
// period was accepted and then produced no slots.
//
// Keep the picker, the validator and computeInterviewSlots reading THESE.
// pages/practice-decision.html cannot import this file (it is a static page),
// so it restates the numbers — a test pins them together.
var INTERVIEW_LEAD_HOURS = 48;
var INTERVIEW_HORIZON_DAYS = 14;

// How long an interview actually runs. This was hardcoded as 45 in eleven
// places across server.js while every scheduled_calls row said 30 (the column
// is NOT NULL DEFAULT 30, and pages/admin.html's Duration select defaults to
// "30 minutes"), so the code looked for a 45-minute hole that nothing had ever
// booked. That is not cosmetic: computeInterviewSlots only emits a start where
// `t + durationMin <= windowEnd`, so a practice offering the exact right
// 30-minute window got ZERO slots and the doctor's card read "No mutually
// available times in the next 2 weeks" (PKG Medical Centre offered 7:00–7:30pm
// on 7 + 8 Sep 2026 and both were silently discarded — owner report
// 2026-09-06). Wherever a practice offered a longer window it merely ate the
// last slot of every window instead, which is why it went unnoticed for months.
//
// Read the ROW's duration_minutes via interviewDurationMinutes() — the length
// is per-interview and staff can change it on the admin form. This constant is
// only the fallback for a row that somehow has none.
var INTERVIEW_DEFAULT_DURATION_MINUTES = 30;

// The stored length of ONE interview, defensively parsed. Anything absent,
// non-numeric or <= 0 falls back to the default rather than poisoning the slot
// maths with a NaN (a NaN durationMin makes `t + NaN <= end` false forever,
// i.e. the same silent "no times" failure this replaces).
function interviewDurationMinutes(row) {
  var n = Number(row && row.duration_minutes);
  return (Number.isFinite(n) && n > 0) ? n : INTERVIEW_DEFAULT_DURATION_MINUTES;
}

// Per-interview override of the 48-hour notice rule, for the case where a
// practice's offered times are genuinely still workable but fall inside the
// standard notice period — an operator waives it deliberately rather than the
// whole system dropping to short notice. Null/absent keeps INTERVIEW_LEAD_HOURS.
// 0 is meaningful (waive entirely) so it must survive the check; slots in the
// past are still excluded by computeInterviewSlots' `t < earliest` test, which
// with 0 notice becomes "not before now".
function interviewLeadHours(row) {
  var n = Number(row && row.min_notice_hours);
  return (Number.isFinite(n) && n >= 0) ? n : INTERVIEW_LEAD_HOURS;
}

// A GUESS, and only ever a fallback. The doctor's browser reports their real
// zone (viewer_tz) and that must win wherever it is available — see
// _interviewComputeSlots' gpTzOverride. This map exists for the paths that have
// no browser to ask: the admin slot view, and emails.
//
// The default matters more than it looks. registration_country is frequently
// EMPTY, and an empty country silently became London — which is fine for the
// overseas doctors this was built for and catastrophic for anyone further east:
// a 10:00 Sydney window reads as 01:00 London, lands outside the assumed waking
// hours, and the whole day is dropped from the doctor's picker with nothing
// logged and nothing shown (owner report 2026-07-31, a real interview that
// offered one day instead of two).
function gpTzForCountry(country) {
  switch (String(country || '').toLowerCase().trim()) {
    ...
    case 'au': case 'aus': case 'australia': return 'Australia/Sydney';
...
function buildInterviewRow(o) {
  ...
  return {
    ...
    // Stated explicitly rather than leaning on the column default, so a row
    // built in local JSON mode carries the same length as one Postgres
    // defaults, and so the number is visible where the row is created.
    duration_minutes: (Number(o.durationMinutes) > 0) ? Number(o.durationMinutes) : INTERVIEW_DEFAULT_DURATION_MINUTES,
    stage: null,
    status: 'invited',
...
module.exports = {
  ...
  INTERVIEW_LEAD_HOURS, INTERVIEW_HORIZON_DAYS,
  INTERVIEW_DEFAULT_DURATION_MINUTES, interviewDurationMinutes, interviewLeadHours,
