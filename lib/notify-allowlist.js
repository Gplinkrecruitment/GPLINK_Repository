...
// Entries are comma-separated; spaces inside a phone number are fine.
//
...
function phoneMatches(a, b) {
  ...
  // A local number (9 significant digits in AU) matches its E.164 form.
...
function parseRecipients(raw) {
  ...
  String(raw || '').split(/[,;\n]+/).map((t) => t.trim()).filter(Boolean).forEach((token) => {
    if (token.indexOf('@') !== -1) emails.add(normaliseEmail(token));
    else {
...
function createNotifyAllowlist(raw) {
  ...
  return { active, size: emails.size + phones.size, permitsEmail, permitsPhone, filterBody, describe, mask: maskRecipient };
}
