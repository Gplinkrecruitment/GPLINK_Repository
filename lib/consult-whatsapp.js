...
const CONSULT_WA_TEMPLATES = {
  ...
  // Touches 2 and 3 of the pre-booking chase (owner request 2026-09-08): the
  // not_booked schedule in lib/consult-lead.js names which step carries which
  // kind. Separate templates so the doctor never reads the same words twice —
  // and, as always, a template still pending WhatsApp approval fails soft.
  not_booked_2: { templateName: 'gp_link_consult_book_nudge_2', language: 'en' },
  not_booked_3: { templateName: 'gp_link_consult_book_nudge_3', language: 'en' },
  ...
  // The "_5m" template says "starts in 5 minutes" (owner request 2026-09-01) —
  // the original gp_link_consult_call_starting said "starting now" and fired at
  // T-15. Timing moved with the wording: see CONSULT_CALL_STARTING_SOON_MS.
  call_starting: { templateName: 'gp_link_consult_call_starting_5m', language: 'en' },
  // The same two touches for an RSO registration-support call (MyIntealth / AMC
  // / AHPRA). A separate pair because it is a different meeting: generic GP Link
  // voice rather than the CEO's, and the audience is app users who may already
  // be in Australia, so the slot is rendered in THEIR timezone.
  reg_call_reminder: { templateName: 'gp_link_reg_call_reminder', language: 'en' },
  reg_call_starting: { templateName: 'gp_link_reg_call_starting', language: 'en' }
};

// Reminder kinds keyed by the window they belong to, per track, so the cron
// never has to translate a window name into a template kind by hand.
const CONSULT_CALL_REMINDER_KIND = { h24: 'call_reminder', soon: 'call_starting' };
const REGISTRATION_CALL_REMINDER_KIND = { h24: 'reg_call_reminder', soon: 'reg_call_starting' };

// The three pre-booking chase touches share one eligibility rule and one
// message shape ([first name, tokenised booking link]); only the template differs.
const NOT_BOOKED_WA_KINDS = ['not_booked', 'not_booked_2', 'not_booked_3'];

// Both tracks build the same two shapes: one that names the slot, one that is
// just "join now".
const REMINDER_KINDS_WITH_TIME = ['call_reminder', 'reg_call_reminder'];
const REMINDER_KINDS_LINK_ONLY = ['call_starting', 'reg_call_starting'];
...
// lands the message before the call begins. This is the DEFAULT (still used by
// the registration-call track); the consult track overrides it below.
const CALL_REMINDER_SOON_MS = 15 * 60 * 1000;
// Owner request 2026-09-01: the consult "starting" message goes out 5 minutes
// before the call, and the template copy says "starts in 5 minutes". With a
// 5-minute window and a 5-minute cron there is exactly ONE tick per booking
// inside the window, so a late or skipped run would silently cost the doctor
// their join link…
const CONSULT_CALL_STARTING_SOON_MS = 5 * 60 * 1000;
// …hence this grace: the soon touch may still go out up to 3 minutes AFTER the
// start time (the copy already says "running a few minutes late is fine"), so
// one delayed cron tick delivers a slightly-late link instead of nothing.
const CONSULT_CALL_STARTING_LATE_GRACE_MS = 3 * 60 * 1000;
...
// The same rendering, but in the timezone the person actually booked in
// (scheduled_calls.timezone, straight from Calendly). Registration-support calls
// reach doctors who may already have moved to Australia, so "(UK time)" would be
// actively misleading for them. Falls back to Europe/London — and to the plain
// UK renderer — when the zone is missing or unusable.
function formatCallTimeInZone(iso, timeZone) {
  const zone = String(timeZone || '').trim();
  if (!zone || zone === 'Europe/London') return formatConsultCallTime(iso);
  const t = new Date(iso || '');
  if (!iso || isNaN(t.getTime())) return '';
  try {
    const date = new Intl.DateTimeFormat('en-GB', {
    ...
    // "AEST" / "GMT+11" — whatever Intl can name the zone, so the doctor can tell
    // at a glance which clock we mean.
    let label = '';
    try {
      const parts = new Intl.DateTimeFormat('en-GB', { timeZone: zone, timeZoneName: 'short' })
        .formatToParts(t);
      const found = parts.find((p) => p.type === 'timeZoneName');
      label = found ? String(found.value || '').trim() : '';
    } catch (_) { label = ''; }
    return date + ', ' + time + (label ? ' (' + label + ')' : '');
  } catch (_) {
    // An unrecognised IANA zone must not cost the doctor their reminder.
    return formatConsultCallTime(iso);
  }
}
...
function consultWaEligible(kind, consult) {
  ...
  if (NOT_BOOKED_WA_KINDS.indexOf(kind) !== -1) {
...
function buildConsultWaMessage(kind, ctx) {
  ...
  if (REMINDER_KINDS_WITH_TIME.indexOf(kind) !== -1) {
    const join = String((ctx && ctx.joinUrl) || '').trim();
    // whenText lets the caller render the slot in the booker's own timezone;
    // without one we fall back to the UK rendering the consult funnel uses.
    const when = String((ctx && ctx.whenText) || '').trim() || formatConsultCallTime(ctx && ctx.callAtIso);
    if (!join || !when) return null;
    return { templateName: tpl.templateName, language: tpl.language, placeholders: [first, when, join] };
  }
  if (REMINDER_KINDS_LINK_ONLY.indexOf(kind) !== -1) {
    const join = String((ctx && ctx.joinUrl) || '').trim();
    if (!join) return null;
...
function consultCallReminderDecision(input) {
  ...
  // Per-track "starting soon" window (consult = 5 min, registration = the
  // 15-min default) and post-start grace (consult only — see the constants).
  const soonMs = Number(input && input.soonMs) || CALL_REMINDER_SOON_MS;
  const lateGraceMs = Number(input && input.lateGraceMs) || 0;
  // Never remind someone about a call that has already begun — detect-no-shows
  // owns everything from the start time onwards. The grace only lets a delayed
  // cron tick deliver the join link moments late, never minutes into the call.
  if (untilMs <= -lateGraceMs) return { action: 'skip', reason: 'already_started' };
  ...
  // Which template pair this booking belongs to — the CEO consultation by
  // default, the RSO registration-support pair when the caller says so.
  const kinds = (input && input.kinds) || CONSULT_CALL_REMINDER_KIND;
  ...
  if (untilMs <= soonMs) {
    ...
    return { action: 'send', window: 'soon', kind: kinds.soon, markers: markers };
  }
  ...
  return { action: 'send', window: 'h24', kind: kinds.h24, markers: markers };
}
...
module.exports = {
  ...
  REGISTRATION_CALL_REMINDER_KIND,
  formatCallTimeInZone,
  ONBOARDING_NUDGE_AFTER_MS,
  ONBOARDING_NUDGE_WINDOW_MS,
  CALL_REMINDER_SOON_MS,
  CONSULT_CALL_STARTING_SOON_MS,
  CONSULT_CALL_STARTING_LATE_GRACE_MS,
