...
function stripHtmlPreview(html) {
  if (!html) return '';
  // End tags: a browser also closes the element on "</script >", "</script\t\n bar>"
  // and "</script/>", so the pattern must allow that trailing junk — otherwise the
  // block is never removed and its raw source leaks into the preview text.
  return String(html)
    .replace(/<style[\s\S]*?<\/style(?:[\s\/][^>]*)?>/gi, ' ')
    .replace(/<script[\s\S]*?<\/script(?:[\s\/][^>]*)?>/gi, ' ')
    .replace(/<[^>]+>/g, ' ')
    // Entities are decoded in ONE pass. Decoding "&amp;" first and "&lt;"/"&gt;"
    // afterwards unescaped twice: the literal text "&amp;lt;" came out as "<".
    .replace(/&(?:nbsp|amp|lt|gt);/gi, function (ent) {
      var k = ent.slice(1, -1).toLowerCase();
      return k === 'nbsp' ? ' ' : k === 'amp' ? '&' : k === 'lt' ? '<' : '>';
    })
    .replace(/\s+/g, ' ').trim();
}
