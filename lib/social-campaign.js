...
// ── Meta Graph ─────────────────────────────────────────────────────────────
// 🧨 FB_PAGE_ACCESS_TOKEN was ONE variable read by two unrelated features with
// different Meta scope requirements: lead hydration needs `leads_retrieval`,
// this publisher needs `pages_manage_posts` and the Instagram scopes. Minting a
// token for one silently broke the other, and it happened in BOTH directions —
// social work killed leads on 2026-08-16, lead work killed posting on 08-18 —
// because the two failures look nothing alike (leads fail loudly by email,
// posting fails quietly in a cron).
//
// Each consumer now has its own variable. FB_PAGE_ACCESS_TOKEN remains as the
// fallback so this is safe to ship before the new variables exist, and so a
// deployment that sets only one of them cannot break the other feature.
function graphConfig(env) {
  ...
  return {
    ...
    // Which variable actually supplied it, so a failure names the one to fix
    // rather than the shared name that is right for neither consumer.
    pageTokenSource: String(e.FB_SOCIAL_PAGE_TOKEN || '').trim()
      ? 'FB_SOCIAL_PAGE_TOKEN'
      : 'FB_PAGE_ACCESS_TOKEN',
    // True while both features are still drawing on the shared value. That state
    // is the bug this split exists to remove, and nothing else says so out loud.
    sharingLeadToken: !String(e.FB_SOCIAL_PAGE_TOKEN || '').trim()
      && !!String(e.FB_PAGE_ACCESS_TOKEN || '').trim(),
...
function graphConfigProblems(cfg, targets) {
  ...
  const out = [];
  if (!cfg.pageToken) out.push((cfg.pageTokenSource || 'FB_PAGE_ACCESS_TOKEN') + ' is not set.');
...
// A credential problem is not the post's fault, and it hits every post equally.
// A malformed or revoked token would otherwise burn three attempts on each
// creative as it came due, quietly retiring a whole month one post at a time
// while the owner saw only "nothing posted today". Held instead, so the run
// resumes by itself once the token is fixed, with no creatives to rescue.
//
// Matched on Meta's wording rather than an error code, because the same
// condition arrives as 190, 102 or 200 depending on the endpoint.
function isCredentialError(message) {
  const m = String(message || '').toLowerCase();
  return (
    m.indexOf('malformed access token') !== -1 ||
    m.indexOf('invalid oauth access token') !== -1 ||
    m.indexOf('session has expired') !== -1 ||
    m.indexOf('access token has expired') !== -1 ||
    m.indexOf('cannot parse access token') !== -1 ||
    m.indexOf('the user has not authorized') !== -1 ||
    m.indexOf('must be called with a page access token') !== -1
  );
}
