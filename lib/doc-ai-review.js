...
// with the expected label — i.e. the model read the file fine and it is simply
// a different document (a bank statement where a degree should be).
function looksLikeWrongDocument(scan) {
  ...
  const expectedTokens = new Set(expected.split(' ').filter(function (w) { return w && !GENERIC_DOC_WORDS.has(w); }));
