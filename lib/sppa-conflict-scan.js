...
async function scanForConflict(params, opts) {
  ...
  // Model selection + automatic upgrade handled centrally (Opus 4.6 → latest
  // supported model if 4.6 is ever retired). See lib/anthropic-model.js.
  ...
  }, { apiKey: apiKey, timeoutMs: 60000 });

  if (!result.ok) {
    return Object.assign(parseConflictScanResponse(''), { _error: result.error, _detail: result.detail });
  }
