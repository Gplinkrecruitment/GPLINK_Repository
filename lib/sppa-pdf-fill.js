...
var { PDFDocument, PDFName, PDFString, PDFArray, PDFRef, PDFDict } = require('pdf-lib');
...
/**
 * "Bake" the current form-field appearances into the page CONTENT of an SPPA-00 PDF.
 *
 * Why: when a candidate/practice fills this PDF, the typed values live only in the
 * interactive form-field (widget annotation) layer. Full PDF viewers (Acrobat,
 * Preview, Chrome) render that layer, but content-only viewers — Gmail's inline
 * preview, Google Drive preview, many email clients — ignore annotations and show
 * the BLANK template. This draws each widget's existing normal appearance onto the
 * page itself so the values are visible in EVERY viewer.
 *
 * Crucially this KEEPS the widgets + AcroForm intact, so the form is still
 * fillable (the practice can still complete Sections B–H after the candidate's
 * Section A is baked in). It also never touches field /V values, so downstream
 * reads (extractAltSupervisorNames, AI scans) still work.
 *
 * Uses only the low-level pdf-lib API per-widget (the high-level form.flatten()
 * crashes on this PDF's malformed fields) and skips any widget that throws.
 *
 * @param {Buffer} pdfBuffer
 * @returns {Promise<{buffer: Buffer, baked: number}>}
 */
async function bakeSppaAppearances(pdfBuffer) {
  function num(x) { return typeof x === 'number' ? x : (x && x.asNumber ? x.asNumber() : Number(x)); }
  var doc = await PDFDocument.load(pdfBuffer, { ignoreEncryption: true });
  var ctx = doc.context;
  var pages = doc.getPages();
  var baked = 0;
  for (var p = 0; p < pages.length; p++) {
    var node = pages[p].node;
    var annots = node.Annots && node.Annots();
    if (!annots || !annots.size || !annots.size()) continue;
    var ops = [];
    var idx = 0;
    for (var i = 0; i < annots.size(); i++) {
      try {
        var aref = annots.get(i);
        var annot = ctx.lookup(aref);
        if (!annot || !annot.get) continue;
        var sub = annot.get(PDFName.of('Subtype'));
        if (!sub || String(sub) !== '/Widget') continue;
        var ap = annot.get(PDFName.of('AP'));
        if (!ap) continue;
        var apDict = ctx.lookup(ap);
        if (!apDict || !apDict.get) continue;
        var nval = apDict.get(PDFName.of('N'));
        var nObj = ctx.lookup(nval);
        var apRef = (nval instanceof PDFRef) ? nval : null;
        var isStream = nObj && nObj.constructor && /Stream/.test(nObj.constructor.name);
        if (!isStream) {
          // checkbox/radio: N is a dictionary of state -> appearance stream; pick /AS
          var as = annot.get(PDFName.of('AS'));
          if (!as || !nObj || !nObj.get) continue;
          var stateVal = nObj.get(as);
          if (!stateVal) continue;
          apRef = (stateVal instanceof PDFRef) ? stateVal : ctx.register(ctx.lookup(stateVal));
          nObj = ctx.lookup(stateVal);
        } else if (!apRef) {
          apRef = ctx.register(nObj);
        }
        if (!nObj || !nObj.dict) continue;
        var bboxArr = nObj.dict.get(PDFName.of('BBox'));
        if (!bboxArr) continue;
        var bx0 = num(ctx.lookup(bboxArr.get(0))), by0 = num(ctx.lookup(bboxArr.get(1)));
        var bx1 = num(ctx.lookup(bboxArr.get(2))), by1 = num(ctx.lookup(bboxArr.get(3)));
        var rectArr = annot.get(PDFName.of('Rect'));
        if (!rectArr) continue;
        var rx0 = num(ctx.lookup(rectArr.get(0))), ry0 = num(ctx.lookup(rectArr.get(1)));
        var rx1 = num(ctx.lookup(rectArr.get(2))), ry1 = num(ctx.lookup(rectArr.get(3)));
        if (rx1 < rx0) { var tmpx = rx0; rx0 = rx1; rx1 = tmpx; }
        if (ry1 < ry0) { var tmpy = ry0; ry0 = ry1; ry1 = tmpy; }
        var bw = (bx1 - bx0) || 1, bh = (by1 - by0) || 1;
        var sx = (rx1 - rx0) / bw, sy = (ry1 - ry0) / bh;
        var tx = rx0 - sx * bx0, ty = ry0 - sy * by0;
        if (!isFinite(sx) || !isFinite(sy) || !isFinite(tx) || !isFinite(ty)) continue;
        var res = node.Resources();
        if (!res) continue;
        var xo = res.lookup(PDFName.of('XObject'));
        if (!xo) { xo = ctx.obj({}); res.set(PDFName.of('XObject'), xo); }
        var name = 'GPLbake' + p + '_' + (idx++);
        xo.set(PDFName.of(name), apRef);
        ops.push('q ' + sx + ' 0 0 ' + sy + ' ' + tx + ' ' + ty + ' cm /' + name + ' Do Q');
        baked++;
      } catch (e) { /* skip malformed widget */ }
    }
    if (ops.length) {
      var streamRef = ctx.register(ctx.stream('\n' + ops.join('\n') + '\n'));
      var contents = node.get(PDFName.of('Contents'));
      if (contents instanceof PDFArray) { contents.push(streamRef); }
      else if (contents) { node.set(PDFName.of('Contents'), ctx.obj([contents, streamRef])); }
      else { node.set(PDFName.of('Contents'), ctx.obj([streamRef])); }
    }
  }
  var outBytes = await doc.save();
  return { buffer: Buffer.from(outBytes), baked: baked };
}

module.exports = { fillSppaQ7, extractAltSupervisorNames, amendSppaField, amendSppaFields, extractSppaFormFields, bakeSppaAppearances, CONFLICT_DETAILS_TEXT };
