...
/**
 * Stamp Q12 values directly onto a SCANNED (flattened) SPPA-00 — a print-and-scan return has
 * no form fields to fill, so the text is drawn onto the page at the template's box position,
...
async function stampSppaQ12OnScan(pdfBuffer, values) {
  ...
  try {
    ...
    if (!wanted.length) return { filled: false, reason: 'nothing_to_stamp', buffer: null, stamped: [] };

    var geo = await _templateFieldGeometry(wanted);
    ...
    for (var w = 0; w < wanted.length; w++) {
      ...
      if (page.getRotation().angle) return { filled: false, reason: 'scan_layout_unknown', buffer: null, stamped: [] };
      var sx = page.getWidth() / g.pageWidth;
      var sy = page.getHeight() / g.pageHeight;
      var x1 = Math.min(g.rect[0], g.rect[2]) * sx;
      var y1 = Math.min(g.rect[1], g.rect[3]) * sy;
      var boxH = Math.abs(g.rect[3] - g.rect[1]) * sy;
      var fontSize = Math.max(8, Math.min(11, boxH - 5));
      var isDate = wanted[w] === Q12_START_DATE_FIELD;
      var text = isDate ? String(values.startDate) : String(values.hoursText);
      // Scanners crop the page edges rather than scaling the print, so pure proportional
      // scaling lands a few points low/left of the printed box — per-field nudges toward the
      // 1:1 position, calibrated against a real practice scan. Placement tolerance is
      // forgiving: the stamp just has to sit legibly in the box like a handwritten entry.
      page.drawText(text, {
        x: x1 + (isDate ? 7 : 9),
        y: y1 + (boxH - fontSize) / 2 + (isDate ? 6 : 10),
        size: fontSize,
        font: font,
        color: rgb(0.1, 0.1, 0.1)
      });
      stamped.push(wanted[w] === Q12_START_DATE_FIELD ? 'start_date' : 'hours');
    }
