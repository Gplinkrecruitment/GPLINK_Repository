 ...
 *                            whiteOutStrayHours?: bool, crossQ14No?: bool, crossQ19Yes?: bool }
...
async function stampSppaQ12OnScan(pdfBuffer, values) {
  ...
  try {
    ...
    if (!wanted.length && !values.whiteOutStrayHours && !values.crossQ14No && !values.crossQ19Yes) {
    ...
    // print → scan round-trip: Q14 NO (kid 1) and Q19 YES (kid 0). The per-cross dy differs
    // because the print is shrunk ~95% ("fit to page"), so the vertical error grows with the
    // box's height on the page — offsets calibrated against a real practice scan.
    var crosses = [];
