...
function buildUploadCheckPrompt(requirement) {
  ...
  var revision = clean(requirement.revision_request, 4000);
  ...
  return [
    ...
    (revision ? '\nOur emails with the sender about this document (what we asked for, and any change they told us to make):\n' + revision : ''),
    '',
    ...
    (revision ? '- Where those emails ask for a specific correction, is EACH one actually made in this file? Say in the summary which were fixed and which were not. Anything still unfixed means "possible_issue".' : ''),
    '',
