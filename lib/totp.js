...
function verifyTotp(secret, token, { window = 1, time = Date.now(), step = 30, digits = 6 } = {}) {
  const candidate = String(token || '').replace(/[\s-]/g, '');
  if (!new RegExp(`^\\d{${digits}}$`).test(candidate)) return false;
  let secretBuf;
  try { secretBuf = base32Decode(secret); } catch (err) { return false; }
  ...
  for (let w = -window; w <= window; w++) {
    ...
    if (crypto.timingSafeEqual(expected, candidateBuf)) matched = true;
  }
  return matched;
}

module.exports = { generateSecret, totpUri, generateTotp, verifyTotp, base32Encode, base32Decode };
