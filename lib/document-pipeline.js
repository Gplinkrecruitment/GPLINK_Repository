...
function classifyQualificationOutcome({ nameMatch, verified }) {
  if (nameMatch === 'mismatch') {
    // A differing name is a suspected NAME CHANGE (e.g. marriage), not a bad document.
    // Still route to a human to confirm, but frame it as a name change to record —
    // never as a "wrong document, re-upload" rejection.
    return { action: 'flag', status: 'under_review', reasonKind: 'name_change' };
  }
