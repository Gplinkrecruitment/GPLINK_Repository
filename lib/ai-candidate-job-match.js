...
function _gpForPrompt(gp) {
  ...
  return {
    ...
    background: String(g.handoverSummary || '').slice(0, 1500)
  };
}
...
async function _callAnthropicRanking(systemPrompt, userPrompt, opts) {
  ...
  // Default to Haiku 4.5 for match ranking: it's a bulk, latency-bounded task
  // (a batch of 25 jobs must return WITHIN the 30s AbortController below AND the
  // whole endpoint within Vercel's 60s function budget). Opus took ~40s for a
  // 25-job batch — tripping the 30s abort — and its reply hit the old 2000-token
  // ceiling, so every batch fell back to null scores + empty reasons (degraded).
  // Haiku 4.5 ranks the same batch in ~11s and fits the token budget. Overridable
  // via ANTHROPIC_MATCH_MODEL.
  var model = o.model || process.env.ANTHROPIC_MATCH_MODEL || 'claude-haiku-4-5';
  ...
  try {
    var resp = await fetchImpl('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: model,
        // Headroom so a full 25-job batch's reasons aren't truncated (the old
        // 2000 ceiling truncated the JSON -> parse_error -> whole batch null).
        // A ceiling, not a target: the model stops at end_turn (~1800 tok for 25
        // jobs), so this doesn't add latency or cost.
        max_tokens: 4000,
