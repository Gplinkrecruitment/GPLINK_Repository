...
// A street line worth masking has a house/unit number or a street-suffix word
// (St, Rd, Ave, ...). A bare town/suburb/city name ("Perth", "Gold Coast") has
// neither and must NOT be treated as a street to strip — it is a PUBLIC place
// name (shown on every job card and allowed in write-ups). Blanket-removing it
// silently corrupts anything that legitimately contains the city: image paths
// like career-hero-images/Perth/Perth 1.jpg (-> // 1.jpg, a 404), masked titles
// ("DPA - Perth - Bulk Billing" -> "DPA -  - Bulk Billing"), and location text.
const STREET_SUFFIX = /\b(st|street|rd|road|ave|avenue|dr|drive|ln|lane|hwy|highway|blvd|boulevard|ct|court|pl|place|cres|crescent|cct|circuit|cl|close|way|terrace|tce|pde|parade|esplanade|esp|grove|walk|row|quay|wharf|arcade)\b/i;
function looksLikeStreetLine(street) {
  if (typeof street !== 'string' || !street.trim()) return false;
  if (/\d/.test(street)) return true;      // house / unit number
  return STREET_SUFFIX.test(street);       // e.g. "Foo Rd" with no number
}
...
function maskIdentity(text, { practiceName, address } = {}) {
  ...
  // Guard against pathologically short names (e.g. a 1-2 char practice_name)
  // that would otherwise match ordinary letters everywhere and mangle the text.
  if (practiceName && typeof practiceName === 'string' && practiceName.trim().length >= 3) {
  ...
  if (address && typeof address === 'string' && address.trim()) {
    ...
    if (street && looksLikeStreetLine(street)) {
