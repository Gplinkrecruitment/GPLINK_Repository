...
// { after: ms, email: bool, wa: <consult-whatsapp kind> | null } — each step
// names which channels it carries. The magic link goes out on qualification
// (see the FB webhook), so every step here is a chase, not the first contact.
// 2h leaves room for someone who is still deciding; 48h is the final EMAIL and
// says so; day 5 is a WhatsApp-only "last note from me" (owner request
// 2026-09-08: one WhatsApp at 2h was too little). Each WhatsApp step is its own
// approved template (lib/consult-whatsapp.js CONSULT_WA_TEMPLATES), so a doctor
// never reads the same message twice.
...
const CONSULT_NUDGE_SCHEDULE_MS = {
  not_booked: [
    { after: 2 * HOUR, email: true, wa: 'not_booked' },
    { after: 48 * HOUR, email: true, wa: 'not_booked_2' },
    { after: 5 * DAY, email: false, wa: 'not_booked_3' },
  ],
  booked_no_signup: [
...
// A not_booked lead whose sequence is being caught up on (a row repaired after a
// mis-stamped stop, a cron outage, a template that was pending approval for a
// day) has several steps overdue at once. Without a floor between them the cron
// would deliver touch 1, 2 and 3 on three consecutive hourly runs — three
// WhatsApps in three hours reads as spam. Steps are therefore never sent closer
// together than this, whatever the schedule says.
const CONSULT_NUDGE_MIN_GAP_MS = 20 * HOUR;
...
function nextConsultNudge(input) {
  ...
  for (let i = 0; i < schedule.length; i++) {
    ...
    // going out waits for the floor (see CONSULT_NUDGE_MIN_GAP_MS). A step with
    // no readable sent_at cannot be spaced against, so it never blocks.
    if (i > 0 && isFinite(lastSentMs) && nowMs - lastSentMs < CONSULT_NUDGE_MIN_GAP_MS) return null;
