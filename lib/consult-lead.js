...
function nextConsultNudge(input) {
  ...
  if (booked) {
    ...
    // (scheduled_at via callAtMs), falling back to the booking time when unknown.
    const bookedAtMs = Date.parse(consult.call_booked_at || '') || createdAtMs;
    const callAtMs = Number(input && input.callAtMs);
    const callAnchorMs = isFinite(callAtMs) ? callAtMs : bookedAtMs;
    for (let i = 0; i < schedule.length; i++) {
      if (sent[seq + ':' + i]) continue;
      const anchorMs = schedule[i].anchor === 'call' ? callAnchorMs : bookedAtMs;
