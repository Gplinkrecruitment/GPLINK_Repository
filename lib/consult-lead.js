...
const RECOGNISED_UNSERVED_COUNTRY_HINTS = [
  'australia', 'india', 'pakistan', 'bangladesh', 'sri lanka', 'nepal', 'south africa',
  ...
  // Explicit "none of your options" answers. Choosing one of these IS a statement.
  'other', 'none of the above', 'elsewhere', 'somewhere else', 'not listed', 'none'
];
...
function resolveFbLeadCountry(map) {
  ...
  if (keyedCountry !== 'other') return { country: keyedCountry, raw: keyed, source: 'question', recognised: true };
  ...
  for (const key of Object.keys(fields)) {
    ...
    if (parsed !== 'other') return { country: parsed, raw: val, source: 'answers', recognised: true };
  }
