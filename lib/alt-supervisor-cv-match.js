...
var { callAnthropicMessages } = require('./anthropic-model.js');
...
async function matchCvToAltSupervisor(cvBuffer, mimeType, altSupervisorNames) {
  ...
  contentParts.push({
    type: 'text',
    text: 'This document is a CV/resume. I need to:\n'
      + '1. Extract the FULL NAME of the person this CV belongs to.\n'
      + '2. Check if their name matches any of these alternate supervisors from an SPPA-00 form:\n'
      + namesStr + '\n\n'
      + 'Return JSON only: {"cv_name": "Full Name", "matched_supervisor": "matching name or null", "confidence": 0.0-1.0}\n'
      + 'Use fuzzy matching (allow middle name differences, title differences like Dr/Prof). confidence should reflect how sure you are of the match.'
  });

  try {
    var aiResult = await callAnthropicMessages({
      max_tokens: 200, temperature: 0,
      messages: [{ role: 'user', content: contentParts }]
    }, { timeoutMs: 25000 });
    if (!aiResult.ok) { console.error('[AltCV Match] AI error:', aiResult.error); return null; }
    ...
    return {
      matched: !!(result.matched_supervisor && result.confidence > 0.5),
      cvName: result.cv_name || '',
      matchedSupervisor: result.matched_supervisor || null,
