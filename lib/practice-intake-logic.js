...
/* A practice states how many GPs it needs as free text on intake ("1", "2",
   "2-3", "Two GPs"). Take the number that keeps the opening open the LONGEST —
   the largest integer present — clamped to [1, 50]. Anything unparseable, or a
   non-intake job with no value, becomes 1, so those openings behave exactly as
   they always have: one hire fills them. */
function parseGpsNeeded(value) {
  const nums = String(value == null ? '' : value).match(/\d+/g);
  let n = 1;
  if (nums && nums.length) n = Math.max.apply(null, nums.map(Number));
  if (!Number.isFinite(n) || n < 1) n = 1;
  if (n > 50) n = 50;
  return n;
}

/* Given the free-text target and how many GPs are already hired on the opening,
   decide whether it is full. Kept pure and separate from the database count so
   the rule is unit-testable. `isFull` is the SINGLE gate for both closing the
   opening (job_status -> 'filled') and redirecting the remaining candidates:
   until it is true, a multi-GP opening stays open and everyone else stays live. */
function computeJobFill(gpsNeededValue, hiredCount) {
  const needed = parseGpsNeeded(gpsNeededValue);
  const hired = Math.max(0, Math.floor(Number(hiredCount) || 0));
  return { needed, hired, isFull: hired >= needed, remaining: Math.max(0, needed - hired) };
}

module.exports = {
  parseSplit, AU_CITIES, nearestCity, abnOk, acnOk, idKind, derivePlace, buildGeneralLocation,
  buildIntakeJobDetails, buildPackageTerms, parseGpsNeeded, computeJobFill,
};
