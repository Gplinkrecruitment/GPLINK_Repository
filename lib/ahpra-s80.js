...
// Drops the quoted history under a reply ("On … wrote:", "> …" lines) so only what the sender
// actually wrote this time is kept.
function stripQuotedReply(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var ln = lines[i];
    if (/^\s*>/.test(ln)) break;
    // "On <date>, <name> wrote:" (sometimes wrapped over the next line or two).
    if (/^\s*On\s/.test(ln) && /\swrote:/.test(lines.slice(i, i + 3).join(' '))) break;
    if (/^\s*-{2,}\s*Original Message\s*-{2,}/i.test(ln) || (out.length && /^\s*From:\s.+/.test(ln))) break;
    out.push(ln);
  }
  return out.join('\n').trim();
}

// What we asked a practice to fix on a resubmitted document, for the advisory AI check: the reason
// a previous copy was rejected, our emails asking for it, and what the practice told us back.
// messages = [{direction:'outbound'|'inbound', body_text, created_at}] (oldest first). Returns ''
// when there is nothing that describes a requested change.
function buildRevisionContext(opts) {
  opts = opts || {};
  var parts = [];
  var reject = cleanString(opts.rejectReason || '', 1000);
  if (reject) parts.push('A previous copy was rejected by our team because: ' + reject);
  var msgs = Array.isArray(opts.messages) ? opts.messages : [];
  var asked = msgs.filter(function (m) { return m && m.direction === 'outbound'; }).slice(-1)[0];
  if (asked) {
    var askedText = stripQuotedReply(asked.body_text).slice(0, 1500);
    if (askedText) parts.push('Our latest email to them asked:\n' + askedText);
  }
  var replies = msgs.filter(function (m) { return m && m.direction === 'inbound'; }).map(function (m) { return stripQuotedReply(m.body_text).slice(0, 800); }).filter(Boolean);
  if (replies.length) parts.push('What they told us in reply (the corrected file should reflect this):\n' + replies.slice(-3).map(function (r) { return '- ' + r.replace(/\s*\n\s*/g, ' '); }).join('\n'));
  return parts.join('\n\n').slice(0, 4000);
}
