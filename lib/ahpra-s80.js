...
function ensureInstructions(meta, ctx) {
  ...
  if (meta.owner === 'gp' && !cleanString(meta.gp_instructions || '', 4000)) {
    var guide = docGuides.matchGuide({ title: item.title, detail: item.detail, kind: item.kind, institution: item.institution }, ctx.country || 'uk');
...
function buildExtractionPrompt(emailMeta, opts) {
  ...
  return [
    ...
    '- "kind": one of "supervised_practice_plan", "qualification_check", "english", "good_standing", "practice_document",',
    '  or "" if none fit.',
    '',
