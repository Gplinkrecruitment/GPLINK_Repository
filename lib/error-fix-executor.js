...
function extractInlineScripts(html) {
  ...
  const out = [];
  // The end tag must allow everything a browser allows: "</script>", "</script >",
  // "</script\t\n bar>" and "</script/>" all close the element. With a narrower
  // pattern the body capture runs on to the NEXT block's end tag, so one block is
  // mis-parsed and the following one is never syntax-checked at all.
