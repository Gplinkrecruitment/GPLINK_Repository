...
var PLACEMENTS_TABLE = 'placements';
...
function createAtsOffersStore(ctx) {
  ...
  // ---- placements (Task C) --------------------------------------------------
  // Placement-of-record row written when a GP accepts an in-app offer. The
  // durable placement data ALSO lives in gp_career_state (same shape the Zoho
  // sync writes), so when the placements table migration (20260703092000)
  // hasn't been applied we TOLERANTLY SKIP the row, cached determination,
  // logged once, instead of falling back to kv. Returns the inserted row, or
  // null when skipped/failed.
  var placementsTableMissing = false;

  function markPlacementsMissing() {
    if (!placementsTableMissing) {
      placementsTableMissing = true;
      // C4: warn (not plain log) so the drift is visible in prod logs, this is
      // exactly the silent 404-skip that masks an unapplied migration.
      console.warn('[ats-offers] placements table missing, migration 20260703092000 may be unapplied (check /api/admin/migration-status). Skipping placement rows; gp_career_state stays the placement of record.');
    }
  }

  async function insertAtsPlacement(row) {
    var placement = row && typeof row === 'object' ? row : null;
    if (!placement) return null;
    // Dedupe: one placement of record per application, so an accept retry /
    // resume can re-run this step idempotently instead of stacking rows.
    var appId = String(placement.application_id || '');

    if (!ctx.isSupabaseDbConfigured()) {
      var db = ctx.getDbState ? ctx.getDbState() : null;
      if (!db) return null;
      if (!Array.isArray(db.atsPlacements)) db.atsPlacements = [];
      if (appId) {
        var priorLocal = db.atsPlacements.find(function (p) { return p && String(p.application_id) === appId; });
        if (priorLocal) return priorLocal;
      }
      var localRow = Object.assign({ id: localOfferId(), created_at: nowIso() }, placement);
      db.atsPlacements.push(localRow);
      ctx.saveDbState();
      return localRow;
    }

    if (placementsTableMissing) return null;
    if (appId) {
      var existing = await ctx.supabaseDbRequest(PLACEMENTS_TABLE, 'select=*&application_id=eq.' + encodeURIComponent(appId) + '&limit=1');
      if (isMissingRelationError(existing)) {
        markPlacementsMissing();
        return null;
      }
      if (existing.ok && Array.isArray(existing.data) && existing.data[0]) return existing.data[0];
      // A failed lookup (network/RLS) still proceeds to insert, losing the
      // placement row would be worse than the rare duplicate it might create.
    }
    var written = await ctx.supabaseDbRequest(PLACEMENTS_TABLE, '', {
      method: 'POST', headers: { Prefer: 'return=representation' }, body: [placement]
    });
    if (isMissingRelationError(written)) {
      markPlacementsMissing();
      return null;
  ...
  // List placements newest-first (bounded). Table-missing is tolerated the same
  // way inserts are: return an empty list (logged once) so the caller can fall
  // back to gp_career_state / gp_applications.
  async function listAtsPlacements(opts) {
    var options = opts && typeof opts === 'object' ? opts : {};
    var limit = Math.max(1, Math.min(Number(options.limit) || 50, 200));

    if (!ctx.isSupabaseDbConfigured()) {
      var db = ctx.getDbState ? ctx.getDbState() : null;
      var rows = (db && Array.isArray(db.atsPlacements)) ? db.atsPlacements.slice() : [];
      rows.sort(function (a, b) {
        return String(b && (b.placed_at || b.created_at) || '').localeCompare(String(a && (a.placed_at || a.created_at) || ''));
      });
      return rows.slice(0, limit);
    }

    if (placementsTableMissing) return [];
    var r = await ctx.supabaseDbRequest(PLACEMENTS_TABLE, 'select=*&order=placed_at.desc&limit=' + limit);
    if (isMissingRelationError(r)) { markPlacementsMissing(); return []; }
    return (r.ok && Array.isArray(r.data)) ? r.data : [];
  }

  // Update the commencement (start_date) on the placement-of-record for an
  // application. Returns the updated row, or null when there is no placement to
  // update / the table is missing.
  async function updateAtsPlacementCommencement(applicationId, startDate) {
    var appId = String(applicationId || '');
    if (!appId) return null;
    var value = startDate || null;

    if (!ctx.isSupabaseDbConfigured()) {
      var db = ctx.getDbState ? ctx.getDbState() : null;
      if (!db || !Array.isArray(db.atsPlacements)) return null;
      var p = db.atsPlacements.find(function (x) { return x && String(x.application_id) === appId; });
      if (!p) return null;
      p.start_date = value;
      p.updated_at = nowIso();
      ctx.saveDbState();
    ...
    if (placementsTableMissing) return null;
    var written = await ctx.supabaseDbRequest(PLACEMENTS_TABLE, 'application_id=eq.' + encodeURIComponent(appId), {
      method: 'PATCH', headers: { Prefer: 'return=representation' }, body: { start_date: value, updated_at: nowIso() }
    });
    if (isMissingRelationError(written)) { markPlacementsMissing(); return null; }
    if (!written.ok) return null;
    var updatedRow = Array.isArray(written.data) && written.data[0] ? written.data[0] : null;
    // A representation PATCH that matched nothing returns []; treat as "no row".
    return updatedRow;
  }
  ...
  return {
    ...
    insertAtsPlacement: insertAtsPlacement,
    listAtsPlacements: listAtsPlacements,
    updateAtsPlacementCommencement: updateAtsPlacementCommencement
  };
}
