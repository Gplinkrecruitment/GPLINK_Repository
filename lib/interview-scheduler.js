...
function partyIntervalsForDate(party, ymd, tz) {
  ...
  // [0, 1440] means "no specific weekend constraint" — fall back to weekday window so
  // the host+GP constraints remain binding (avoids generating early-morning or late-night
  // slots when the other two parties have tighter schedules).
  if (weekend && win && win[0] === 0 && win[1] === 1440) win = party.weekday;
